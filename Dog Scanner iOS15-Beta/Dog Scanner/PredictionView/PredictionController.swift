...
//  Dog Scanner
//
//  Created by Michael Spilger on 15.06.21.
//
...
class PredictionController: ObservableObject {
    ...
    @ObservedObject var model = ModelDataHandler(modelFileInfo: NeuralNet.modelInfo, labelsFileInfo: NeuralNet.labelsInfo)!
    
    @Published var prediction: [Inference] = []
    
    func predict(image: UIImage) -> [Inference]? {
        let img = pixelBufferFromImage(image: image)
        let pred = model.runModel(onFrame: img)
        prediction = pred!
        return pred
    }
}
